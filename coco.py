...
"""
import os, time, numpy as np, torch, zipfile, urllib.request, shutil, argparse
from mylibs import MaskRCNN, General_Dataset
...
class CocoDataset(General_Dataset):
    ...
    def auto_download(self, dataDir, dataType, dataYear):
        ...
        mkdir_if_missing(dataDir)           # Create main folder if it doesn't exist yet

        # Download images if not available locally
        if not is_path_exists(imgDir):
            mkdir_if_missing(imgDir)
            print("Downloading images to " + imgZipFile + " ...")
            with urllib.request.urlopen(imgURL) as resp, open(imgZipFile, 'wb') as out: shutil.copyfileobj(resp, out)
            print("... done downloading.")
            print("Unzipping " + imgZipFile)
            with zipfile.ZipFile(imgZipFile, "r") as zip_ref: zip_ref.extractall(dataDir)
        ...
        mkdir_if_missing(annDir)
        if not is_path_exists(annFile):
            if not is_path_exists(annZipFile):
                print("Downloading zipped annotations to " + annZipFile + " ...")
                with urllib.request.urlopen(annURL) as resp, open(annZipFile, 'wb') as out: shutil.copyfileobj(resp, out)
                print("... done downloading.")
            print("Unzipping " + annZipFile)
            with zipfile.ZipFile(annZipFile, "r") as zip_ref: zip_ref.extractall(unZipDir)
            print("... done unzipping")
...
if __name__ == '__main__':
    ...
    # Create model
    if args.command == "train": model = MaskRCNN(config=config, model_dir=args.logs)
    else: model = MaskRCNN(config=config, model_dir=args.logs)
